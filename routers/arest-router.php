...
include '../includes/connect.php';
	foreach ($_POST as $key => $value)
	{
        ...
        if(preg_match("/[0-9]+_tuesday/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET tue = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_tuesdayclose/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET tuec = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_wednesday/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET wed = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_wednesdayclose/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET wedc = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_thursday/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET thurs = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_thursdayclose/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET thursc = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_friday/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET fri = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_fridayclose/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET fric = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_saturday/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET sat = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_saturdayclose/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET satc = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_sunday/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET sun = '$value' WHERE id = $key;";
			$con->query($sql);
		}
        if(preg_match("/[0-9]+_sundayclose/",$key)){
			$key = strtok($key, '_');
			$sql = "UPDATE users SET sunc = '$value' WHERE id = $key;";
			$con->query($sql);
		}
